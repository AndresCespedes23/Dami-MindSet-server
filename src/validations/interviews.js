...
const required = (req, res, next) => {
  if (!req.body.idCandidate || !req.body.idClient || !req.body.idPosition || !req.body.status) {
...
const validate = (req, res, next) => {
  ...
  if (!idClient) {
    return res.status(400).json({ msg: "idClient is required" });
  }
  if (!idPosition) {
    return res.status(400).json({ msg: "idPosition is required" });
  }
