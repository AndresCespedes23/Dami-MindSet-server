...
const validateUpdate = (req, res, next) => {
  ...
  if (idCandidate) {
    if (idCandidate._id.search(/[a-z]/) < 0) return res.status(400).json({ msg: "idCandidate must have one letter at least" });
    if (idCandidate._id.search(/[0-9]/) < 0) return res.status(400).json({ msg: "idCandidate must have one number at least" });
  }
  if (idClient) {
    if (idClient._id.search(/[a-z]/) < 0) return res.status(400).json({ msg: "idClient must have one letter at least" });
    if (idClient._id.search(/[0-9]/) < 0) return res.status(400).json({ msg: "idClient must have one number at least" });
  }
  if (idPosition) {
    if (idPosition._id.search(/[a-z]/) < 0) return res.status(400).json({ msg: "idPosition must have one letter at least" });
    if (idPosition._id.search(/[0-9]/) < 0) return res.status(400).json({ msg: "idPosition must have one number at least" });
  }
