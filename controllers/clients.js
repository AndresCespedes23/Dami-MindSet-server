...
const getByPhoneNumber = (req, res) => {
  const client = clients.find(client => client.PhoneNumber === req.params.PhoneNumber);
    if (client) return res.status(200).json(client);
    return res.status(404).send("Client not found");
}
...
const getByAddress = (req, res) => {
  const client = clients.find(client => client.address === req.params.address);
  if (client) return res.status(200).json(client);
  return res.status(404).send("Client not found");
}

const getByActivity = (req, res) => {
  const client = clients.find(client => client.activity === req.params.activity);
  if (client) return res.status(200).json(client);
  return res.status(404).send("Client not found");
}
...
const create = (req, res) => {
  if(!req.query.name ||
     ...
     !req.query.cuit ||
     !req.query.address ||
     !req.query.activity) return res.status(400).send('Some parameters are missing');
  const newClient = {
